...
def spi_disabled():
    return shell.run_command("sudo raspi-config nonint get_spi", suppress_message=True,
                             return_output=True).strip() == "1"
...
@click.option('--reboot', nargs=1, default=None, type=click.Choice(['yes', 'no']),
              help="Specify whether to reboot after the script is finished")
...
""")
    ...
    if os_identifier != "Raspbian":
        shell.bail(
            "Sorry, the OS detected was {}. This script currently only runs on Raspberry Pi OS.".format(os_identifier))
    ...
    if menu_selection == 1:
        while True:
            ...
            if ce0_pin is not None:
                ...
                ce1_pin = None
            if shell.prompt(
                    f"The new settings will be {format_gpio(ce0_pin)} for CE0 and {format_gpio(ce1_pin)} for CE1. "
                    f"Is this correct?"):
                break
        remove_custom()
